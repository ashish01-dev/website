...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col h-screen w-60 fixed left-0 top-0 bg-black border-r border-white/[0.06] py-2 select-none liquid-glass">
      <div className="px-3 py-2 mb-1">
        <div className="text-sm font-semibold text-white">{settings.name || 'User'}</div>
        <div className="text-[11px] text-white/50">JEE 2027</div>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-2 px-2 py-1 rounded-lg text-sm transition-colors ${
                isActive ? 'bg-white/[0.08] text-white' : 'text-white/50 hover:text-white hover:bg-white/[0.05]'
              }`}
            >
      ...
      {loaded && (
        <div className="px-3 pt-2 mt-2 border-t border-white/[0.06]">
          <div className="text-[10px] text-white/50 uppercase tracking-wider mb-2 font-medium">Chapter Progress</div>
                ...
                <div key={sub} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between text-[10px]">
                      <span className="text-white/50 capitalize truncate">{sub}</span>
                      <span className="text-white font-medium ml-1">{stats.done}/{stats.total}</span>
                    </div>
