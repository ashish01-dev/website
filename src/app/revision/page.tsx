  ...
  return (
    <div className="min-h-screen pb-[100px] md:pb-[90px]" style={{ fontFamily: "'DM Sans', sans-serif", background: 'linear-gradient(to top left, #F5F5F5, #F7F7F7)' }}>
      ...
      <div className="max-w-[900px] mx-auto px-4 md:px-6 py-8">
        <h1 className="text-[clamp(28px,3vw,36px)] font-medium tracking-[-0.5px] mb-1" style={{ color: '#0f0f0f' }}>Revision Hub</h1>
        <p className="text-sm mb-6" style={{ color: '#888' }}>Error log and formula vault</p>

        <div className="flex items-center gap-1 mb-6 border-b pb-0" style={{ borderColor: 'rgba(0,0,0,0.06)' }}>
          <button onClick={() => setTab('errors')} className={`px-3 py-2 text-sm border-b-2 -mb-[1px] transition-colors ${tab === 'errors' ? 'border-[#2383e2] text-[#2383e2] font-medium' : 'border-transparent hover:text-[#0f0f0f]'}`} style={{ color: tab !== 'errors' ? '#888' : undefined }}>
            Error Log
          </button>
          <button onClick={() => setTab('formulas')} className={`px-3 py-2 text-sm border-b-2 -mb-[1px] transition-colors ${tab === 'formulas' ? 'border-[#2383e2] text-[#2383e2] font-medium' : 'border-transparent hover:text-[#0f0f0f]'}`} style={{ color: tab !== 'formulas' ? '#888' : undefined }}>
            Formula Vault
          </button>
        </div>
        ...
        {tab === 'errors' && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="rounded-[18px] p-4" style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              <h2 className="text-[15px] font-semibold mb-4" style={{ color: '#0f0f0f' }}>Log an Error</h2>
              <div className="space-y-3">
                ...
                <div>
                  <label className="text-[10px] font-semibold uppercase tracking-wider block mb-1" style={{ color: '#888' }}>QUESTION</label>
                ...
                <div>
                  <label className="text-[10px] font-semibold uppercase tracking-wider block mb-1" style={{ color: '#888' }}>WHY WRONG</label>
                ...
                }} className="w-full flex items-center justify-center text-sm font-medium px-4 py-2 rounded-[40px] text-white" style={{ background: 'linear-gradient(180deg, #2c2c2c 0%, #111111 100%)' }}>Log Error</button>
              </div>
            </div>
            <div className="md:col-span-2 rounded-[18px] p-4" style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
              <h2 className="text-[15px] font-semibold mb-4" style={{ color: '#0f0f0f' }}>Recent Errors</h2>
              {errorEntries.length === 0 ? (
                <p className="text-sm text-center py-8" style={{ color: '#888' }}>No errors logged yet.</p>
              ) : (
                <div className="space-y-2 max-h-[400px] overflow-y-auto">
                  {[...errorEntries].reverse().slice(0, 20).map(e => {
                    ...
                    return (
                      <div key={e.id} className="p-3 rounded-[12px]" style={{ background: '#fafafa', border: '1px solid rgba(0,0,0,0.06)' }}>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs" style={{ color: '#888' }}>{chName}</span>
                          <span className="text-[10px] ml-auto" style={{ color: '#888' }}>{e.date}</span>
                        </div>
                        <p className="text-sm mb-0.5" style={{ color: '#0f0f0f' }}>{e.question}</p>
                        {e.reason && <p className="text-xs" style={{ color: '#e03e3e' }}>{e.reason}</p>}
                      </div>
                    )
                  })}
                </div>
          ...
          <div>
              ...
              {(['physics', 'chemistry', 'maths'] as Subject[]).map(s => (
                <button
                  ...
                  className={`px-3 py-2 text-sm border-b-2 -mb-[1px] transition-colors ${
                    subjectTab === s ? 'border-[#2383e2] text-[#2383e2] font-medium' : 'border-transparent hover:text-[#0f0f0f]'
                  }`}
                  style={{ color: subjectTab !== s ? '#888' : undefined }}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {currentChapters.map(ch => {
                ...
                return (
                  <div key={ch.id} className="rounded-[18px] p-4" style={{ background: '#fff', border: '1px solid rgba(0,0,0,0.05)', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
                    <div className="flex items-start justify-between mb-2">
                      <div className="min-w-0 flex-1">
                        <div className="text-[10px] font-semibold uppercase tracking-wider mb-0.5" style={{ color: '#2383e2' }}>Class {ch.class} &middot; {ch.weightage}</div>
                        <h3 className="text-sm font-medium" style={{ color: '#0f0f0f' }}>{ch.name}</h3>
                      </div>
                      <span className="text-xs ml-2 whitespace-nowrap" style={{ color: '#888' }}>{files.length} file{files.length !== 1 ? 's' : ''}</span>
                    </div>
                    ...
                    {files.length > 0 && (
                      <div className="space-y-1.5 mb-3">
                        {files.map((file, idx) => {
                          ...
                          return (
                            <div key={idx} className="flex items-center gap-2 px-2 py-1.5 rounded-[10px]" style={{ background: '#fafafa' }}>
                              {isImage && file.url && (
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="text-xs truncate" style={{ color: '#0f0f0f' }}>{file.name}</div>
                                <div className="text-[10px]" style={{ color: '#888' }}>{formatFileSize(file.size)}</div>
                              </div>
                              {file.url && (
                                <a href={file.url} target="_blank" rel="noopener noreferrer" className="text-[10px] hover:underline flex-shrink-0" style={{ color: '#2383e2' }}>Open</a>
                              )}
                              <button onClick={() => deleteFile(ch.id, idx)} className="text-[10px] hover:underline flex-shrink-0" style={{ color: '#e03e3e' }}>Delete</button>
                            </div>
                          )
                        })}
                      </div>
                    ...
                    {files.length === 0 && (
                      <p className="text-xs mb-3 italic" style={{ color: '#888' }}>No files uploaded yet.</p>
                    )}
                    ...
                    <button
                      ...
                      style={{ border: '1px solid rgba(0,0,0,0.1)', color: '#2383e2' }}
                    >
